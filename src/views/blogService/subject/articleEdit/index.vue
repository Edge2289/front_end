<template>
  <el-form ref="form" :model="form" label-width="80px" style="padding: 30px;">
    ...
    display: block;"
      ...
      <ImgLibrary :dialog-visible="dialogVisible" @changeDialogVisible="changeDialogVisible" />
    </el-form-item>
    <br />
    <el-form-item label="文章分类">
      <el-select v-model="form.cate_id" placeholder="请选择文章分类">
        <el-option v-for="(item, key) in cate_list" :label="item.name" :value="item.id" :key="key"></el-option>
        <el-option label="区域二" value="beijing"></el-option>
      </el-select>
    </el-form-item>
    <br />
    <el-form-item label="标签">
      <el-checkbox-group v-model="form.labels" class="tab_list_border">
        <el-checkbox
      ...
      if (!(articleId == undefined || articleId == '')) {
        getArticleList({
          ...
          response => {
            ...
            this.form.describe = response.data.list[0].describe;
            this.form.is_comment = response.data.list[0].is_comment;
            this.form.is_state = response.data.list[0].is_state;
            this.form.cate_id = response.data.list[0].cate_id;
            this.form.labels = response.data.list[0].cate_id;
            this.form.url = response.data.list[0].img;
            this.form.text = response.data.list[0].text_data.text;
            this.form.markdown = response.data.list[0].text_data.markdown;
            this.content = response.data.list[0].text_data.markdown;
            let labels = [];
            response.data.list[0].label_data.forEach(function(item , index){
