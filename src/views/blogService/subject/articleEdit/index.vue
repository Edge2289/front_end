<template>
  <el-form ref="form" :model="form" label-width="80px" style="padding: 30px">
    ...
    <el-form-item
      ...
      "
    >
      <el-button type="primary" @click="onSubmit" v-loading="sureLoading">立即保存</el-button>
...
export default {
  ...
  methods: {
    ...
    onSubmit() {
      ...
      // 赋值
      let queryVal = JSON.parse(JSON.stringify(this.form) );
      queryVal.label_data = queryVal.label_data.join(",");
      requestHeader(queryVal).then((response) => {
        if (response.code == 200) {
          this.msgSuccess(response.msg);
          setTimeout(this.shutDownWin(), 2000);
        } else {
