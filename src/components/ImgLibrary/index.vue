  ...
  <el-dialog v-dialogDrag title="图片库" :visible.sync="dialogVisible" width="840px" :close-on-click-modal="false" :show-close="false">
    <el-container style="height: 450px; border: 1px solid #eee;border:none;line-height: 16px;">
      <el-aside class="img_menu" width="150px" style="font-size: 14px;padding: 0px;margin-top: 18px;background: white;">
        <div :id="-1" :class="{ img_menu_xuanzhong: searchSelectedGroup == -1 }" style="margin-left: 12px;cursor: pointer;" @click="selectGroup(-1)" @mouseover="changeActive($event)" @mouseout="removeActive($event)">全部图片</div>
        <div :id="0" :class="{ img_menu_xuanzhong: searchSelectedGroup == 0 }" style="margin-left: 12px;cursor: pointer;" @click="selectGroup(0)" @mouseover="changeActive($event)" @mouseout="removeActive($event)">未分组</div>
        <div v-for="(itemGroup, indexGroup) in groupList" :id="itemGroup.id" :key="indexGroup" :class="{ img_menu_xuanzhong: searchSelectedGroup == itemGroup.id }" style="cursor: pointer;" @click="selectGroup(itemGroup.id)" @mouseover="changeActive($event)" @mouseout="removeActive($event)">
            ...
            <el-col :span="12">
              <el-dropdown trigger="click" @command="mvImgsGroup">
                ...
                <el-dropdown-menu slot="dropdown">
                  ...
                  <el-dropdown-item v-for="(item, key) in groupList" :id="item.id" :key="key" :command="item.id">{{ item.name }}</el-dropdown-item>
                </el-dropdown-menu>
              </el-dropdown>
              <el-button type="danger" size="mini" @click="delImgs">删除</el-button>
            </el-col>
